...
static void tzdev_put_user_pages(struct mm_struct *mm, struct page **pages,
		unsigned long nr_pages)
{
	unsigned long i;
...
static int __tzdev_mem_free(int id, void *p, void *data)
{
	...
	tzdev_put_user_pages(mem->mm, mem->pages, mem->nr_pages);
	mmput(mem->mm);
...
#define TZDEV_PFNS_PER_PAGE	(PAGE_SIZE / sizeof(sk_pfn_t))
	...
	/* mm reference is kept till mem removal */
	mm = get_task_mm(task);
	if (!mm) {
		ret = -ESRCH;
		goto out_task;
...
out_pin:
	tzdev_put_user_pages(mm, pages, nr_pages);
out_mem:
	kfree(mem);
out_pfns:
	kfree(pfns);
...
static long tzdev_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TZIO_WAIT_EVT: {
		ret = wait_for_completion_interruptible(&tzdev_ow_comp);
		break;
	}
	case TZIO_GET_PIPE: {
		struct timespec ts;
		__s32 dta;
		__s32 __user *argp = (__s32 __user *)arg;
		getnstimeofday(&ts);
