...
{
	if(do_action){
		__mxt_patch_ddebug(data, "|-- TCFG_WRITE: OBJECT_TYPE:%d"
			" OFFSET:%d VAL:%d\n",
...
{
	if(do_action){
		__mxt_patch_ddebug(data, "|-- ECFG_WRITE: OBJECT_TYPE:%d"
			" OFFSET:%d VAL:%d\n",
	...
	if(do_action){
		switch(action_id){
			case MXT_PATCH_ACTION_NONE:
				__mxt_patch_ddebug(data, "|-- ACTION NONE\n");
				break;
			...
			u8* pmsg, u8 offset, u16 mask)
{
	...
	if(pmsg){
		...
		if(offset < MXT_PATCH_MAX_MSG_SIZE){
