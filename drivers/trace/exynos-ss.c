...
#ifdef CONFIG_SEC_DEBUG
		sec_getlog_supply_kernel((void*)phys_to_virt(ess_items[ESS_ITEMS_LOG_KERNEL].entry.paddr));
#endif
...
static int __init exynos_ss_fixmap(void)
{
	...
	for (i = 1; i < ARRAY_SIZE(ess_items); i++) {
		...
		if (!strncmp(ess_items[i].name, "log_kernel", strlen(ess_items[i].name))) {
			...
			printk("sec_debug: LAST_LOGBUF(last_buf:%zx, align:%zx, paddr_align:%zx\n", last_buf, align, (paddr & 0xFF000000));
