...
static void fg_vbatocv_check(struct i2c_client *client)
{
	...
	if(((abs(fuelgauge->info.batt_current)<40) || ((fuelgauge->is_charging) && (abs(fuelgauge->info.batt_current)<200))) && (abs(fuelgauge->info.p_batt_current-fuelgauge->info.batt_current)<10))
	{
...
static SIMPLE_DEV_PM_OPS(sm5703_fuelgauge_pm_ops, sm5703_fuelgauge_suspend,
		sm5703_fuelgauge_resume);
